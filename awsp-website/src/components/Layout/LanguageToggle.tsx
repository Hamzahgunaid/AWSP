  ...
  return (
    <button
      ...
      onMouseEnter={e => (e.currentTarget as HTMLElement).style.background =
        onDark ? 'rgba(255,255,255,0.1)' : 'var(--bone)'}
      onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}
    >
      <span style={{ color: locale === 'ar'
        ? (onDark ? 'var(--teal-300)' : 'var(--teal-600)') : 'inherit' }}>AR</span>
      <span style={{ opacity: 0.4 }}>|</span>
      <span style={{ color: locale === 'en'
        ? (onDark ? 'var(--teal-300)' : 'var(--teal-600)') : 'inherit' }}>EN</span>
    </button>
  );
}
