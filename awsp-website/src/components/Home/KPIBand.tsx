...
export default function KPIBand({ locale }: { locale: string }) {
  ...
  return (
    <section style={{ background: 'var(--ink-800)', color: '#fff', padding: '70px 0', position: 'relative', overflow: 'hidden' }}>
      {/* shimmer top line */}
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '1px', background: 'linear-gradient(90deg, transparent, var(--teal-400), var(--sand-400), transparent)', opacity: 0.5 }} />
      <div className="wrap">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 0 }}>
          {KPIS.map((k, i) => (
            <div key={i} style={{ padding: '8px 24px', borderInlineEnd: i < 4 ? '1px solid rgba(255,255,255,0.1)' : 'none' }}>
              <div style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2.6rem, 4.5vw, 4rem)', fontWeight: '400', lineHeight: 1, color: '#fff', letterSpacing: '-0.02em', display: 'flex', alignItems: 'baseline', gap: '4px' }}>
